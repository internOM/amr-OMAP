...
import os
import sys
from tfLib import euler_from_quaternion, quaternion_from_euler
...
class MobiconSubscription(Node):
    ...
    def response_callback(self, res):
        ...
        if (res.slave_id == 1) & (res.func_code == 0x17):
            ...
            update_quaternion = quaternion_from_euler(0,0,float(_yaw))
            odom_pose.orientation.x = update_quaternion[0]
            odom_pose.orientation.y = update_quaternion[1]
            odom_pose.orientation.z = update_quaternion[2]
            odom_pose.orientation.w = update_quaternion[3]
            ...
            map_frame.transform.rotation.w = odom_pose.orientation.x
            map_frame.transform.rotation.x = odom_pose.orientation.y
            map_frame.transform.rotation.y = odom_pose.orientation.z
            map_frame.transform.rotation.z = odom_pose.orientation.w
...
class MobiconPublisher(Node):
    ...
    def set_data(self):
        ...
        msg.data[8] = 0  # Read data[8] → 
        msg.data[9] = 0  # Read data[9] → 
