...
class ImgReceiver(Node):
	...
	def listener_callback(self, data):
		...
		ret, binary = cv2.threshold(mask, 150, 255, cv2.THRESH_BINARY)
		countours, hierarchy = cv2.findContours(binary, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
		img_blank = np.ones_like(frame)
		img_countour_only = cv2.drawContours(img_blank, countours, -1, (255,0,0), 3)
		cv2.imshow('MwqrASKED',img_blank)
