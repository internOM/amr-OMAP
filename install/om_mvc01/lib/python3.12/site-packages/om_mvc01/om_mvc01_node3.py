...
class MVC01Contoroller(Node):
    ...
    def response_callback(self, res):
        ...
        if (res.slave_id == 1) & (res.func_code == 0x17):
            ...
            self.last_odom_header.stamp = self.get_clock().now().to_msg()
            #odom_header.frame_id = "map"
            #odom_header.child_frame_id = "base_link"
            self.last_odom_pose.position.x = float(res.data[0]/1000.0)
            self.last_odom_pose.position.y = float(res.data[1]/1000.0)
            self.last_odom_pose.position.z = float(res.data[2]/1000000.0)
            update_quaternion = quaternion_from_euler(0,0,float(res.data[2]/1000000.0))
            self.last_odom_pose.orientation.w = update_quaternion[0]
            self.last_odom_pose.orientation.x = update_quaternion[1]
            self.last_odom_pose.orientation.y = update_quaternion[2]
            self.last_odom_pose.orientation.z = update_quaternion[3]
            
            self.last_odom_twist.linear.x = float(res.data[5]/1000.0)
            self.last_odom_twist.linear.y = float(res.data[7]/1000.0)
            self.last_odom_twist.angular.z = float(res.data[6]/1000000.0)
            ...
            map_frame.transform.translation.x = self.last_odom_pose.position.x
            map_frame.transform.translation.y = self.last_odom_pose.position.y
            map_frame.transform.translation.z = self.last_odom_pose.position.z
            map_frame.transform.rotation.w = self.last_odom_pose.orientation.w
            map_frame.transform.rotation.x = self.last_odom_pose.orientation.x
            map_frame.transform.rotation.y = self.last_odom_pose.orientation.y
            map_frame.transform.rotation.z = self.last_odom_pose.orientation.z
    ...
    def cmdVel_callback(self, msg):
        self.last_cmdvel.linear.x = msg.linear.x
        self.last_cmdvel.linear.y = msg.linear.y
        self.last_cmdvel.angular.z = msg.angular.z
    ...
    def set_drive_operation(self):
        global msg
        #print("{:20}: {:10}".format("Speed Vx [m/s]", self.last_cmdvel.linear.x))
        #print("{:20}: {:10}".format("Speed Vy [m/s]", self.last_cmdvel.linear.y))
        #print("{:20}: {:10}".format("Speed ω [rad/s]", self.last_cmdvel.angular.z))
        # vω制御モードで動作さMVC01Contorollerせる
        ...
        msg.data[1] = int(self.last_cmdvel.linear.x * 1000.0)  # DD前後並進速度
        msg.data[2] = int(self.last_cmdvel.angular.z * 1000000.0) # DD角速度
        msg.data[3] = int(self.last_cmdvel.linear.y * 1000.0)  # DD左右並進速度
        msg.data[4] = 1  # 反映トリガ
        # 配信
...
def main(args=None):
    ...
    mvc01Ctl = MVC01Contoroller()
    try:
        rclpy.spin(mvc01Ctl)
    except KeyboardInterrupt:
        pass
    finally:
        mvc01Ctl.destroy_node()
        rclpy.shutdown()
